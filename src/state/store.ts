...
export interface AppState {
  ...
  setCameraMode: (mode: 'default' | 'focus') => void;
...
export const useAppStore = create<AppState>((set) => ({
  ...
  finishBoot: () => 
    set({ isBooting: false, bootProgress: 100, showDesktop: true, booted: true, focused: true, cameraMode: 'focus' }),
  
  setBooted: (booted: boolean) => set({ booted }),
  
  setFocused: (focused: boolean) => set({ focused }),
  
  setHoveringMonitor: (hovering: boolean) => 
    set((state) => {
      // If hovering and booted, focus camera
      if (hovering && state.booted) {
        return { hoveringMonitor: hovering, cameraMode: 'focus', focused: true };
      }
      // If not hovering and no active windows, unfocus
      if (!hovering && Object.keys(state.windows).length === 0) {
        return { hoveringMonitor: hovering, cameraMode: 'default', focused: false };
      }
      return { hoveringMonitor: hovering };
    }),
  
  setCameraMode: (mode: 'default' | 'focus') => set({ cameraMode: mode }),
